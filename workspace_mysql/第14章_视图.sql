...
①视图，可以看作是一个虚拟表，本身是不存储数据的。视图的本质，就可以看作是
 存储起来的select语句
...
CREATE VIEW vu_emp2
AS
SELECT employee_id emp_id,last_name lname,salary #查询语句中字段的别名会作为视图中字段的名字
FROM emps;
...
AS
SELECT employee_id e,e.department_id ,d.department_name
