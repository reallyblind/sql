...
DESC myemp1;
#查看创建表的语句结构
SHOW CREATE TABLE myemp1;#如果创建表时没有指明使用的字符集，则默认使用表中所在数据库的字符集
...
#8.对比 TRUNCATE TABLE(DDL) 和 DELECT FROM(DML)
#相同点：都可以实现对表中所有数据的删除，同时保留表结构
#不同点：
#	TRUNCATE TABLE：一旦执行此操作，表数据全部清除。同时，数据是不可以回滚的
#	DELECT FROM：一旦执行此操作，表数据可以全部清除(不带WHERE)。同时，数据是可以实现回滚的


/*
9.DDL 和 DML 的说明
① DDL的操作一旦执行，就不可回滚。指令SET autocommit = FALSE对DDL操作失效(因为在执行完DDL
   操作之后,一定会执行一次COMMIT.此COMMIT操作不受SET autocommit = FALSE影响)
