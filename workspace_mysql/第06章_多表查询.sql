...
#6.结论：如果有n个表实现多表查询，则需要至少n-1个连接条件
...
#外连接：合并具有同一列的两个以上的表的行, 结果集中中除了包含一个表与另一个表匹配的行之外，
#  	还查询到了左表 或 右表中不匹配的行

#外连接的分类：左外连接、右外连接、满外连接

#左（右）外连接：两个表在连接过程中除了返回满足连接条件的行以外还返回左（或右）表中不满足条件的
...
#SQL99语法中使用JOIN...ON 的方式实现多表查询。这种方法也能解决外连接的问题。MySQL支持此种方式的多表查询
#SQL99语法如何实现多表的查询
...
#NATURAL JOIN:他会帮你自动查询两张连接表中'所有相同的字段'
SELECT employee_id,last_name,department_name
